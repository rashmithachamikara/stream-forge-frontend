...
function KpiCard({
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 text-left">
      ...
      <p className="text-2xl font-bold tracking-tight text-foreground font-mono">{isLoading ? '-' : value}</p>
      {!isLoading && (
        <p className={cn(
          "text-[11px] mt-1 font-medium font-mono inline-flex items-center gap-1",
...
export default function EditorAnalyticsDashboard() {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    const [
      ...
      apiClient.getMeAnalyticsSummary(analyticsRange),
      apiClient.getMeAnalyticsSummary(prevRange),
      apiClient.getMeViewsOverTime(analyticsRange),
      apiClient.getMeBreakdown('device', analyticsRange),
      apiClient.getMeBreakdown('browser', analyticsRange),
      apiClient.getMeBreakdown('category', analyticsRange),
      apiClient.getMeBreakdown('tag', analyticsRange),
      apiClient.getMeAuthBreakdown(analyticsRange),
    ]);
  ...
  const loadRankedVideos = useCallback(async () => {
    ...
    const response = await apiClient.getMeRankedVideos(apiKind, { ...analyticsRange, page: 1, pageSize: 10 });
  ...
  const handleExport = async () => {
    ...
    const response = await apiClient.downloadMeAnalyticsVideos(analyticsRange);
  ...
  return (
    <DashboardLayout title="Creator Analytics" requiredRoles={['editor', 'admin']}>
      <div className="space-y-8">
        {/* Header Controls Strip */}
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between border-b border-border pb-5 text-left">
          <div>
            <p className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Editor · Analytics</p>
            <h1 className="text-2xl font-bold tracking-tight mt-1 text-foreground">My video performance</h1>
          </div>
          
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono uppercase text-muted-foreground">Presets:</span>
              <Select defaultValue="30d" onValueChange={handlePresetSelectChange}>
        ...
        {error && (
          <div className="border border-destructive/20 bg-destructive/5 rounded-lg p-4 text-sm text-destructive text-left">
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px] text-left">
          {/* Views Over Time */}
          ...
          {/* Real-Time Stats (Average session duration and completion counts) */}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 text-left">
          ...
          <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6 flex flex-col">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5 border-b border-border pb-4">
              <div>
                <h3 className="text-sm font-semibold text-foreground">Video performance rankings</h3>
          ...
          {/* Month Heatmap Card + Day of Week bar chart stacked */}
          <div className="flex flex-col gap-6 h-full lg:col-span-1">
                  ...
                  <div>
                          ...
                          {Array.from({ length: 7 }).map((_, di) => {
                            ...
                            return (
                              <Tooltip key={di}>
                                <TooltipTrigger asChild>
                                  <div
                                    className={`h-[10px] w-full rounded-[2px] cursor-default transition-all duration-150 hover:ring-1 hover:ring-primary/60 ${cell.value === 0 ? 'border border-border' : ''}`}
                                    style={cell.value > 0 ? { backgroundColor: `color-mix(in oklch, var(--primary) ${Math.round(cell.intensity * 100)}%, transparent)` } : undefined}
                                  />
                                </TooltipTrigger>
                    ...
                    <div className="flex items-center gap-1.5 mt-3">
                      ...
                      {[0.07, 0.25, 0.5, 0.75, 1].map((op) => (
                        <div key={op} className="h-[10px] w-[10px] rounded-[2px]" style={{ backgroundColor: `color-mix(in oklch, var(--primary) ${Math.round(op * 100)}%, transparent)` }} />
                      ))}
            ...
            {dayOfWeekData.length > 0 && (
              <div className="bg-card border border-border rounded-xl p-6">
                <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground mb-4">Views by day of week</p>
        ...
        {/* Demographics Breakdowns grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 text-left">
          <BreakdownCard title="Devices" items={deviceBreakdown} emptyLabel="No device stats" isLoading={isLoading} />
          <BreakdownCard title="Browsers" items={browserBreakdown} emptyLabel="No browser stats" isLoading={isLoading} />
          <BreakdownCard title="Viewer Type" items={authItems} emptyLabel="No viewer stats" isLoading={isLoading} />
          <BreakdownCard title="Categories" items={categoryBreakdown} emptyLabel="No category stats" isLoading={isLoading} />
          <BreakdownCard title="Tags" items={tagBreakdown} emptyLabel="No tag stats" isLoading={isLoading} />
        </div>
      </div>
    </DashboardLayout>
  );
